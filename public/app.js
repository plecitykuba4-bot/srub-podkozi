...
async function boot(){const me=await api('me');state={...state,...me,date:workday(me.clock.date),view:me.user?.role==='admin'?'dashboard':'menu',dirty:false,filter:'all'};if(me.demo&&me.user)state.devUsers=(await api('dev-users')).users;await render();}
